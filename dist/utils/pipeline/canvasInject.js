...
exports.injectBrowserCanvasChart = injectBrowserCanvasChart;
/**
 ...
 */
async function injectBrowserCanvasChart(page, violations) {
    ...
    await page.evaluate((errors) => {
        ...
        if (!ctx)
            return;
        const occupiedLabelsRegistry = [];
        errors.forEach((err) => {
            const el = document.querySelector(err.selector);
            if (!el || err.selector === 'html' || err.selector === 'body')
                return;
