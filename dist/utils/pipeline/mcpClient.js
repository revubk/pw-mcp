...
`.trim();
        const sanitizeModelOutput = (raw) => {
            ...
            const relevant = collected.filter((stmt) => stmt.startsWith('await ') || stmt.startsWith('const ') || stmt.startsWith('let ') || stmt.startsWith('var ') || stmt.startsWith('//'));
        ...
        if (geminiApiKey) {
            try {
                ...
                const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/${geminiModel}:generateContent?key=${geminiApiKey}`, {
                ...
                const candidateText = response.data?.candidates?.[0]?.content?.parts?.map((p) => p.text).join('\n') || '';
        ...
        if (!generatedActionLines && lmStudioModel) {
            try {
                ...
                const response = await axios_1.default.post(lmStudioChatEndpoint, {
                    model: lmStudioModel,
                    messages: [
                        {
                            role: 'system',
