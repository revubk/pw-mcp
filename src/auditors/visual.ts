 ...
 */
export async function runVisualAudit(page: Page, url: string, reportDir: string): Promise<VisualAuditResult[]> {
  ...
  for (const vp of VIEWPORTS) {
    console.log(`📸 [Visual Engine] Testing ${vp.name} (${vp.width}x${vp.height}) for: ${url}`);
    ...
    if (!fs.existsSync(baselinePath)) {
      console.log(`   🆕 [Baseline] No baseline found for ${vp.name}. Saving current view as golden master.`);
    ...
    try {
      ...
      console.log(`   ✅ [Visual Pass] ${vp.name} layout matches baseline perfectly.`);
      results.push({
        viewport: vp.name,
        status: 'PASSED',
        baselinePath,
