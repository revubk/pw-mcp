...
export async function injectVisualColorsChart(
  page: Page,
  violations: any[],
): Promise<void> {
  ...
  for (const error of violations) {
    ...
    if (sel && sel !== "html" && sel !== "body" && sel !== "main") {
      if (!uniqueCategoryColorMap[ruleId]) {
        uniqueCategoryColorMap[ruleId] =
          categoryColorsPalette[
            assignedColorsCount % categoryColorsPalette.length
          ];
        assignedColorsCount++;
      }
        ...
        if ((await elementLocator.count()) > 0) {
          await elementLocator.evaluate(
            (el, config) => {
              ...
              if (htmlEl.parentElement) htmlEl.parentElement.appendChild(badge);
              else htmlEl.appendChild(badge);
            },
            {
              ruleId,
              color: activeCategoryColor,
              index: activeOccurrenceIndex,
            },
          );
        }
      } catch (_) {}
