...
export class WebCrawler {
  ...
  public async startCrawl(
    ...
    ) => Promise<{ a11yErrors: number; seoScore: number }>,
    ...
    while (this.queue.length > 0 && !this.isForceStopped) {
      ...
      try {
        ...
        const auditMetrics = await onPageDiscover(
          page,
          cleanCurrentUrl,
          status,
        ...
        if (isBroken || auditMetrics.a11yErrors > 0) {
          const fileSafeName = cleanCurrentUrl
            .replace(/[^a-z0-9]/gi, "_")
            .toLowerCase()
            .substring(0, 50);
          ...
          for (let i = 0; i < count; i++) {
            ...
            if (href) {
              try {
                ...
                if (
                  cleanAbsoluteUrl.startsWith(this.baseUrl) &&
                  !this.visitedUrls.has(cleanAbsoluteUrl) &&
                  !this.queue.includes(cleanAbsoluteUrl)
                ) {
